...
function _generateEntityItemsGraphicalMove(_itemInstance = undefined, _fromEntity = undefined, _toEntity = undefined, _filter = undefined) {
    ...
    if (entityGiveItem(_fromEntity, _toEntity, _itemInstance)) {
...
function _generateEntityItemsMenuMove(_itemInstance, _fromEntity = undefined, _toEntity = undefined, _useLastMenu = false, _switch = false, _allowSwitch = true, _filter = undefined) {
    ...
    var _gaveItem = false;
    if (_switch) {
        if (debug) console.log("  Switch: {0} is giving {1} to {2}".format(_fromEntity.id, _itemInstance.parent.id, _toEntity.id));
        _gaveItem = entityGiveItem(_fromEntity, _toEntity, _itemInstance, _useLastMenu);
    }
    else {
        if (debug) console.log("  NoSwitch: {0} is giving {1} to {2}".format(_fromEntity.id, _itemInstance.parent.id, _toEntity.id));
        _gaveItem = entityGiveItem(_fromEntity, _toEntity, _itemInstance, _useLastMenu);
    }
    ...
    if (_gaveItem) {
        ...
        if (_toEntity == player) {
            if (_fromEntity instanceof Character)
                Content.add("<p>{0} take{1} the {2} from {3}.</p>".format(subjectPronoun(true).capitalize(), pov == 3 ? "s" : "", _itemInstance.parent.toString(), _fromEntity.toString()));
            else if (_fromEntity instanceof Furniture)
                Content.add("<p>{0} take{1} the {2} from the {3}.</p>".format(subjectPronoun(true).capitalize(), pov == 3 ? "s" : "", _itemInstance.parent.toString(), _fromEntity.toString()));
        }
 ...
 * @return Boolean
 */
function giveItem(_itemInstance, _fromEntity) {return entityGiveItem(_fromEntity, player, _itemInstance, true);}
/**
 * Passes time.
...
function characterDisrobeItem(_character, _itemInstance, _executeScene = false) {
    ...
    if (unsafeExec("{0}Disrobe({1}, {2})".format(_itemInstance.parent.id, _character.id, _executeScene))) {
...
function characterGiveItem(_character, _entity, _itemInstance, _executeScene = false) {
    ...
    if (!entityRemoveItem(_character, _itemInstance, _executeScene))
        return false;
    if (unsafeExec("{0}Give({1}, {2}, {3})".format(_itemInstance.parent.id, _character.id, _entity.id, _executeScene))) {
        return entityAddItem(_character, _entity, _itemInstance, _executeScene);
    }
    return false;
}
function characterHoldItem(_character, _itemInstance, _hand = undefined, _executeScene = false) {
    ...
    if (unsafeExec("{0}Hold({1}, {2})".format(_itemInstance.parent.id, _character.id, _executeScene))) {
...
function characterReleaseItem(_character, _itemInstance, _hand = undefined, _executeScene = false) {
    ...
    if (unsafeExec("{0}Release({1}, {2})".format(_itemInstance.parent.id, _character.id, _executeScene))) {
...
/**
 * Adds _itemInstance to _character from _entity
...
function characterAddItem(_character, _entity, _itemInstance, _executeScene = false) {
    ...
    if (unsafeExec("{0}Take({1}, {2}, {3})".format(_itemInstance.parent.id, _character.id, _entity.id, _executeScene))) {
...
function characterTakeItem(_character, _entity, _itemInstance, _executeScene = false) {
    return characterAddItem(_character, _entity, _itemInstance, _executeScene);
}
...
function entityAddItem(_fromEntity, _toEntity, _itemInstance, _executeScene = false) {
    ...
    if (_toEntity instanceof Character)
        return characterAddItem(_toEntity, _fromEntity, _itemInstance, _executeScene);
    else if (_toEntity instanceof Furniture)
        return furnitureAddItem(_toEntity, _fromEntity, _itemInstance, _executeScene);
    else if (_toEntity instanceof Clothing)
        return clothingAddItem(_toEntity, _fromEntity, _itemInstance, _executeScene);
    else
        return false;
}
/**
 ...
 * @return {Boolean}
 */
function entityGiveItem(_fromEntity = undefined, _toEntity = undefined, _itemInstance = undefined, _useLastMenu = false) {
    var _gaveItem = false;
    ...
    if (_fromEntity instanceof Character)
        ...
        if (unsafeExec("{0}Remove({1}, {2})".format(_itemInstance.parent.id, _fromEntity.id, _executeScene)))
            _fromEntity.removeItem(_itemInstance);
        else
            return _gaveItem;
        if (unsafeExec("{0}Take({1}, {2}, {3})".format(_itemInstance.parent.id, _fromEntity.id, _toEntity.id, _executeScene)))
            _gaveItem = characterAddItem(_toEntity, _fromEntity, _itemInstance, _executeScene);
    }
    else if (_fromEntity instanceof Clothing)
        _gaveItem = clothingTakeItem(_toEntity, _fromEntity, _itemInstance, _executeScene);
...
function _executeItemRemoveEvents(_itemInstance, _fromEntity, _toEntity) {
    ...
    if (typeof _fromEntity != 'undefined') {
        if (_fromEntity instanceof Character) {
            ...
            if (!unsafeExec("{0}Remove({1})".format(_itemInstance.parent.id, _fromEntity.id)))
                return false;
            else if (!unsafeExec("{0}Remove({1})".format(_fromEntity.id, _itemInstance.parent.id))) // mimic :o
                return false;
        }
...
function _executeItemGiveEvents(_itemInstance, _fromEntity, _toEntity) {
    ...
    if (_fromEntity instanceof Character && !characterGiveItem(_fromEntity, _toEntity, _itemInstance.parent, (_fromEntity == player || _toEntity == player)))
        return false;
...
function entityRemoveItem(_entity, _itemInstance, _executeScene = false) {
    ...
    if (_entity instanceof Character)
        return characterRemoveItem(_entity, _itemInstance, _executeScene);
    else if (_entity instanceof Furniture)
        return furnitureRemoveItem(_entity, _itemInstance, _executeScene);
    else if (_entity instanceof Clothing)
        return clothingRemoveItem(_entity, _itemInstance, _executeScene);
    else
        return false;
}
function furnitureAddItem(_furniture, _entity, _itemInstance, _executeScene = false) {
    ...
    if (unsafeExec("{0}Take({1}, {2}, {3})".format(_itemInstance.parent.id, _furniture.id, _entity.id, _executeScene))) {
        _furniture.addItem(_itemInstance);
        return true;
    }
    else
        return false;
    ...
    _character.items.forEach(function(_itemInstance) {
        if (_itemInstance.parent instanceof Cheque) {
            if (_itemInstance.parent.to instanceof Character && (_itemInstance.parent.signed || _itemInstance.parent.to == _character) && !isNaN(_itemInstance.parent.amount) && _itemInstance.parent.amount > 0) {
                characterIncMoney(_itemInstance.parent.to, _itemInstance.parent.amount);
                _character.removeItem(_itemInstance);
